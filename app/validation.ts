export function checkForMissingData(input, config) {
  ...
  const errors = configKeys.reduce((cumm: any, data) => {
...
export function validate(input, config) {
  ...
  const errors = Object.keys(input).reduce((cumm: any, value) => {
...
function doValidation(key, input, config) {
  const validation = Object.keys(config[key]).reduce((cumm: any, value) => {
